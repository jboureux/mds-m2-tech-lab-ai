...
export default async function AdminModerationPage() {
	...
	const flaggedCount = await prisma.post.count({
		where: { status: "FLAGGED" },
	});
	const removedCount = await prisma.post.count({
							...
							{flaggedPosts.map((post) => (
								...
								>
									...
									<TableCell className="align-top">
										<Badge
											variant="outline"
											className="bg-destructive/10 text-destructive border-destructive/20 font-bold text-[10px] px-2 py-0"
										>
											Toxic Content
										</Badge>
									</TableCell>
